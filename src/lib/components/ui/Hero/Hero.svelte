...
>
	{#if level === 'primary'}
		...
		></div>
		<div
			class="pointer-events-none absolute top-0 right-0 h-5 w-5 border-t-2 border-r-2 border-secondary"
			aria-hidden="true"
		></div>
		<div
			class="pointer-events-none absolute right-0 bottom-0 h-5 w-5 border-r-2 border-b-2 border-secondary"
			aria-hidden="true"
		></div>
			...
			<div class="flex w-full flex-col gap-10 lg:flex-row">
				<div class="flex flex-1 flex-col items-start gap-10">
					<h1 class="vanchi-display text-5xl text-base-content lg:text-7xl">
						{segments.before}<span class="font-serif text-base-content italic"
							>{segments.match}</span
						>{segments.after}
					</h1>
					{#if subtitle}
