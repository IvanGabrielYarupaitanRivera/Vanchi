...
>
		...
		>
			...
			<div class="flex w-full flex-col items-stretch gap-4 sm:w-auto sm:flex-row sm:items-center">
				{#if action_label && action_onclick}
					<button onclick={action_onclick} type="button" class="btn uppercase btn-primary">
						{action_label}
					</button>
				{/if}
				{#if action_secondary_label && action_secondary_onclick}
					<button
						...
						type="button"
						class="btn uppercase btn-outline"
					>
						{action_secondary_label}
					</button>
				{/if}
	...
	{:else if level === 'secondary'}
		<div
			class="mx-auto grid max-w-5xl grid-cols-1 items-end gap-8 border-x border-base-300 bg-base-100 px-6 py-20 md:grid-cols-12 md:px-12 lg:py-32"
		>
			...
			{#if subtitle}
				<div class="pb-1 md:col-span-5">
					...
					{#if action_label && action_onclick}
						<button onclick={action_onclick} type="button" class="btn mt-6 uppercase btn-outline">
							{action_label}
						</button>
					{/if}
				</div>
